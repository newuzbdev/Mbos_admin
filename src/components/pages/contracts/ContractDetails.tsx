...
import { useGetContract } from "@/hooks/useContract"; // Import your hook
import { useEffect } from "react";

const ContractDetails = () => {
  const { contractId } = useParams<{ contractId: string }>();
  const { data, isLoading, isError } = useGetContract(contractId); // Adjust this to fetch by ID

  useEffect(() => {
    // You might want to add error handling or loading states here
  }, [data]);

  if (isLoading) return <div>Loading...</div>;
  if (isError) return <div>Error fetching contract details.</div>;

  return (
    <div className="p-4">
      <h1 className="text-lg font-bold">Contract Details</h1>
      <div className="mt-4">
        <p><strong>Client:</strong> {data?.data.user.F_I_O}</p>
        <p><strong>Contract Type:</strong> {data?.data.shartnoma_turi}</p>
        <p><strong>Price:</strong> {data?.data.price}</p>
        <p><strong>Advance Payment:</strong> {data?.data.advancePayment}</p>
        <p><strong>Remaining Payment:</strong> {data?.data.remainingPayment}</p>
        <p><strong>Purchase Status:</strong> {data?.data.purchase_status}</p>
        <p><strong>Quantity:</strong> {data?.data.count}</p>
